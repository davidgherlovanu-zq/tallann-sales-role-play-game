...
import { conversationId, sessionStartTime, scoreHistory, setScoreHistory } from './state.js';
...
export async function saveScorecard(data) {
  ...
  try {
    const scorecardData = {
      ...
      conversationId, userEmail: auth.currentUser.email, userId: auth.currentUser.uid
    };
    await db.collection('users').doc(auth.currentUser.uid).collection('scorecards').add(scorecardData);
    try { await db.collection('all_scorecards').add(scorecardData); } catch(e) { console.warn('Could not save to all_scorecards:', e.message); }
...
export async function loadScoreHistory() {
  ...
  try {
    ...
    snap.forEach(doc => {
      const d = doc.data();
      history.push({ id: doc.id, overall_score: d.overall_score, overall_grade: d.overall_grade,
        summary: d.summary, criteria: d.criteria, strengths: d.strengths, improvements: d.improvements,
        coaching_tip: d.coaching_tip, createdAt: d.createdAt ? d.createdAt.toDate() : new Date() });
    });
    setScoreHistory(history);
    renderHistoryList();
    const activeTab = document.querySelector('[data-tab="history"]');
    if (activeTab && activeTab.classList.contains('active')) renderHistoryChart();
