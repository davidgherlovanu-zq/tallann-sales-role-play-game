...
export function listenForAuth() {
	...
	auth.onAuthStateChanged(async (user) => {
		...
		if (user) {
			...
			showApp();
			notifyBubbleAuthStatus("Authenticated");
		} else {
			setCurrentUser(null);
			showAuthScreen();
			notifyBubbleAuthStatus("Unauthenticated", "initial_load");
		}
	});
}
...
export function toggleAuthMode() {
	...
	if (els.title)
		els.title.textContent = newIsSignUp ? "Create Account" : "Welcome Back";
	if (els.subtitle)
		els.subtitle.textContent = newIsSignUp
	...
	if (els.toggleText)
		els.toggleText.textContent = newIsSignUp
			? "Already have an account?"
			: "Don't have an account?";
	if (els.toggleLink)
		els.toggleLink.textContent = newIsSignUp ? "Sign in" : "Create one";
	if (els.nameField)
		els.nameField.style.display = newIsSignUp ? "block" : "none";
	if (els.resetLink)
		els.resetLink.style.display = newIsSignUp ? "none" : "block";
